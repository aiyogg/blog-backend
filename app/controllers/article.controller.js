 ...
 * @return null
 * */
function refreshTagUsedNum(req, res, next) {
  Tags.find({}, {
    '_id': 1
  }, function (err, tags) {
    if (err) {
...
module.exports = {
  ...
  getAllWithPages: function (req, res, next) {
    ...
    Articles.find({
      state: true
    }).sort('-publish_time').skip(from).limit(limit).populate({
      path: 'tags',
      select: 'name'
    }).exec(function (err, docs) {
      if (err) {
  ...
  getByTagId: function (req, res, next) {
    ...
    Articles.find({
      ...
      state: true
    }).skip(from).limit(limit).populate({
      path: 'tags',
      select: 'name'
    }).exec(function (err, docs) {
      if (err) {
  ...
  getLatestTop: function (req, res, next) {
    ...
    Articles.find({
      ...
      'read_num': 1,
      'publish_time': 1
    }).sort('-publish_time').limit(parseInt(req.params.topNum)).exec(function (err, docs) {
      console.log(err);
      res.status(200);
      res.send({
        'code': '1',
  ...
  getReadTop: function (req, res, next) {
    ...
    Articles.find({
      ...
      'title': 1,
      'read_num': 1
    }).sort('-read_num').limit(parseInt(req.params.topNum)).exec(function (err, docs) {
      console.log(err);
      res.status(200);
      res.send({
        'code': '1',
